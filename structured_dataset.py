...
class StructuredDataset(Dataset):
    ...
    def validate_dataset(self):
        ...
        for f in [self.features, self.protected_attributes, self.labels,
            ...
            if not isinstance(f, np.ndarray):
                raise TypeError(
                    "'{}' must be an np.ndarray.".format(f.__name__))

        # convert ndarrays to float64
        self.features = self.features.astype(np.float64)
        self.protected_attributes = self.protected_attributes.astype(
            np.float64)
        ...
        try:
            ...
            print("ValueError: {}".format(e))
            raise ValueError(
                "'scores' should have the same shape as 'labels'.")
        ...
        if np.any(np.logical_or(self.scores < 0., self.scores > 1.)):
            warning(
                "'scores' has no well-defined meaning out of range [0, 1].")
    ...
    def align_datasets(self, other):
        ...
        new.protected_attribute_names = deepcopy(
    ...
    def export_dataset(self, export_metadata=False):
        ...
        if export_metadata:
            raise NotImplementedError(
                "The option to export metadata has not been implemented yet")
    ...
    def import_dataset(self, import_metadata=False):
        ...
        if import_metadata:
            raise NotImplementedError(
                "The option to import metadata has not been implemented yet")
        return None
