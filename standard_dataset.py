...
class StandardDataset(BinaryLabelDataset):
    ...
    def __init__(self, df, label_name, favorable_classes,
        ...
        categorical_features = sorted(
            set(categorical_features) - set(features_to_drop), key=df.columns.get_loc)
        ...
        for attr, vals in zip(protected_attribute_names, privileged_classes):
            ...
            if callable(vals):
                ...
                priv = np.logical_or.reduce(
                    np.equal.outer(vals, df[attr].to_numpy()))
        ...
        if callable(favorable_classes):
            ...
            unfavorable_label = set(df[label_name]).difference(
                favorable_classes).pop()
        else:
        ...
        super(StandardDataset, self).__init__(df=df, label_names=[label_name],
                                              ...
                                              scores_names=[
                                                  scores_name] if scores_name else [],
